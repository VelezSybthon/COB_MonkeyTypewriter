...
(async () => {
  ...
  assert(document.querySelector('.page-foot')?.textContent.includes('v1.0.4'), '训练页底部显示版本 v1.0.4');
  window.location.hash = '#/chinese-training';
  window.dispatchEvent(new window.HashChangeEvent('hashchange'));
  assert(document.querySelector('.page-foot')?.textContent.includes('v1.0.4'), '中文训练页底部显示版本号');
  window.location.hash = '#/new-practice-1';
  window.dispatchEvent(new window.HashChangeEvent('hashchange'));
  assert(document.querySelector('.page-foot')?.textContent.includes('v1.0.4'), '占位页底部显示版本号');
